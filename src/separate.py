...
output_dir = '../results/separate/'
if not os.path.isdir(output_dir):
    os.makedirs(output_dir)

# read in images
image1 = "../images/panda.jpg"
I1 = imread(image1, flatten=True).astype(np.float64) / 255
print I1.shape

image2 = "../images/hedgehog.jpg"
I2 = imread(image2, flatten=True).astype(np.float64) / 255
print I2.shape
...
A = np.array([[np.sqrt(2), np.sqrt(2)], [-np.sqrt(2), np.sqrt(2)]]) / 2
